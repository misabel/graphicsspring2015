...
Scene* Parser::parseScene()
{
  ...
  for( ;; )
  {
    ...
    switch( t->kind() )
    {
      ...
      case MATERIAL:
		 {
		   // can either define a material by name 
		   // material { name="gold"; ... }
		   // or define materials inside a mesh
		   // box { material = "gold"; ... }
		   _tokenizer.CondRead(EQUALS);
           auto_ptr<Material> temp( parseMaterialDeclaration( scene, *mat ));
		   mat = temp;
		 }
         break;
      case SEMICOLON:
...
Material* Parser::parseMaterialDeclaration(Scene* scene, const Material& parent)
{
	_tokenizer.Read(MATERIAL);
	Material* mat = parseMaterial(scene, parent);
	_tokenizer.CondRead(SEMICOLON);
...
Material* Parser::parseMaterial( Scene* scene, const Material& parent )
{
  ...
  if( IDENT == tok->kind() )
  {
	 return new Material(materials[(_tokenizer.Read(IDENT))->ident()]);
  }
