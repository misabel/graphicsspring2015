							  ...
							  const float fMinY, const float fMaxY)
{
	...
	// sometimes when clicking to the left of the red line when there are no control points, 
	// a new control point is not created. Moving this phantom point causes a crash unless we
	// explicitly test for a control point count of 0.
	if (iCtrlPtCount == 0) {
		return;
	}
